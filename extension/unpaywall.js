...
function findDoiFromNumber() {
  ...
  return runRegexOnDoc(
    /Document Object Identifier \(DOI\): (10.*?)<\/p>/,
    "www.nber.org"
  );
}
...
function reportInstallation() {
  ...
  if (loc.indexOf("unpaywall.org") === 0) {
    ...
    $("<div style='display:none' id='unpaywall-is-installed'></div>").appendTo(
      "body"
    );
  }
}
...
function runWithDelay() {
  ...
  var longDelayHosts = ["psycnet.apa.org"];
